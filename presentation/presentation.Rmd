...
---
```{r, include=FALSE}
my_spdf <- readOGR("/cloud/project/presentation/presentation_files/tectonicplates-master/PB2002_plates.shp")
```

```{r, echo=FALSE}
beatCol <- colorFactor(palette = 'RdYlGn', factor(volcanic_eruption$vei))

leaflet(volcanic_eruption) %>% 
  addTiles() %>%
  addCircleMarkers(
    ~longitude.x, ~latitude.x,
    radius = ~sqrt(volcanic_eruption$vei*5),
...
```{r}
leaflet(subset(volcanic_eruption, vei ==2)) %>% 
  addTiles() %>%
  addCircleMarkers(
    ~longitude.x, ~latitude.x,
    radius = 3,
...
```{r tec-settings, echo = FALSE, out.width = "40%", fig.align = "center", fig.cap = "Image credit: Oregon Department of Geology and Mineral Industries- Plate Tectonics & Hot Spots"} 
...
```{r eruptions-by-year-crust, echo=FALSE}
#eruption_tecs%>%
 # group_by(start_year)%>%
 # count(start_year)
...
## Comparison
.pullright[
```{r tec-map, out.width="50%",echo=FALSE}
```
]
.pullleft[
```{r crust-map, out.width="50%", echo=FALSE}
```
]
